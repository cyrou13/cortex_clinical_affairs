...
export class PubMedClient implements DatabaseClient {
  ...
  private async esearch(query: string): Promise<{ idList: string[]; count: number }> {
    ...
    if (!result) {
      throw new Error('PubMed esearch: missing esearchresult');
    }
    if (result.ERROR) {
  ...
  private async fetchArticles(pmids: string[]): Promise<ArticleMetadata[]> {
  ...
  parseEfetchXml(xml: string): ArticleMetadata[] {
    ...
    for (const block of articleBlocks) {
      ...
      if (pmidMatch?.[1]) article.pmid = pmidMatch[1];
      ...
      while ((abstractMatch = abstractRegex.exec(block)) !== null) {
        if (abstractMatch[1]) abstractParts.push(stripXmlTags(abstractMatch[1]).trim());
      }
      if (abstractParts.length > 0) article.abstract = abstractParts.join(' ');
      ...
      while ((authorMatch = authorRegex.exec(block)) !== null) {
        ...
        if (lastName || foreName) authors.push(`${lastName} ${foreName}`.trim());
      }
