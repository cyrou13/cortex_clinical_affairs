...
import { PipelineProgressBar, type PipelineNodeStatus } from '../../../../shared/components/PipelineProgressBar';
...
export default function ProjectDashboardPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-12">
        <p className="text-[var(--cortex-text-muted)]">Loading project dashboard...</p>
      </div>
    );
  }

  if (error || !data?.projectDashboard) {
    return (
      <div className="flex items-center justify-center p-12">
        <p className="text-red-500">Failed to load project dashboard.</p>
      </div>
    );
  }
  ...
  const pipelineNodes = [
    ...
    { id: 'validation', label: 'Validation', status: mapModuleStatus(dashboard.pipelineStatus.validation) },
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[var(--cortex-text-secondary)]">
          {dashboard.deviceName} — Class {dashboard.deviceClass} — {contextLabels[dashboard.regulatoryContext] ?? dashboard.regulatoryContext}
        </p>
      </div>

      {/* Pipeline */}
      <div className="rounded-lg border border-[var(--cortex-border)] bg-white p-4">
        <h2 className="mb-3 text-sm font-medium text-[var(--cortex-text-secondary)]">Pipeline Status</h2>
        <PipelineProgressBar nodes={pipelineNodes} />
      </div>
        ...
        <div className="rounded-lg border border-[var(--cortex-border)] bg-white p-4">
          ...
          <dl className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <dd className="font-medium">{contextLabels[dashboard.regulatoryContext] ?? dashboard.regulatoryContext}</dd>
            </div>
            <div className="flex justify-between">
            ...
            {dashboard.members.map((member) => (
              <div key={member.id} className="flex items-center gap-3 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 text-xs text-[var(--cortex-text-muted)]">{member.role}</span>
                </div>
              </div>
            ))}
          </div>
