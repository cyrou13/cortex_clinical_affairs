...
export function TrendAnalysisPanel({ cycleId }: TrendAnalysisPanelProps) {
  ...
  const [baseForm, setBaseForm] = useState({ periodStart: '', periodEnd: '', totalUnitsShipped: 0, activeDevices: 0 });
  const [conclusions, setConclusions] = useState('');

  const { data: baseData, loading: baseLoading } = useQuery<{ installedBaseEntries: InstalledBaseEntry[] }>(
    GET_INSTALLED_BASE_ENTRIES,
    { variables: { cycleId } },
  );
      ...
      <section data-testid="installed-base-section">
        ...
        {showBaseForm && (
          <div className="mb-4 grid grid-cols-2 gap-3 rounded-lg border border-[var(--cortex-border)] bg-white p-4">
            <input type="date" value={baseForm.periodStart} onChange={(e) => setBaseForm((f) => ({ ...f, periodStart: e.target.value }))} className="rounded-md border border-[var(--cortex-border)] px-3 py-2 text-sm" placeholder="Period Start" data-testid="base-period-start" />
            <input type="date" value={baseForm.periodEnd} onChange={(e) => setBaseForm((f) => ({ ...f, periodEnd: e.target.value }))} className="rounded-md border border-[var(--cortex-border)] px-3 py-2 text-sm" placeholder="Period End" data-testid="base-period-end" />
            <input type="number" value={baseForm.totalUnitsShipped} onChange={(e) => setBaseForm((f) => ({ ...f, totalUnitsShipped: parseInt(e.target.value) || 0 }))} className="rounded-md border border-[var(--cortex-border)] px-3 py-2 text-sm" placeholder="Units Shipped" data-testid="base-units-shipped" />
            <input type="number" value={baseForm.activeDevices} onChange={(e) => setBaseForm((f) => ({ ...f, activeDevices: parseInt(e.target.value) || 0 }))} className="rounded-md border border-[var(--cortex-border)] px-3 py-2 text-sm" placeholder="Active Devices" data-testid="base-active-devices" />
            <div className="col-span-2 flex justify-end gap-2">
              <button type="button" onClick={() => setShowBaseForm(false)} className="rounded-md border border-[var(--cortex-border)] px-3 py-1.5 text-sm text-[var(--cortex-text-secondary)]">Cancel</button>
              <button type="button" onClick={handleAddEntry} className="rounded-md bg-[var(--cortex-blue-500)] px-3 py-1.5 text-sm font-medium text-white hover:bg-[var(--cortex-blue-600)]" data-testid="save-base-btn">Save</button>
            </div>
          </div>
        )}
        ...
        {entries.length === 0 ? (
          ...
          <div className="overflow-x-auto rounded-lg border border-[var(--cortex-border)]">
            <table className="w-full text-sm">
              ...
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} className="border-t border-[var(--cortex-border)]" data-testid={`base-entry-${entry.id}`}>
                    <td className="px-4 py-2 text-[var(--cortex-text-primary)]">{entry.periodStart}</td>
                    <td className="px-4 py-2 text-[var(--cortex-text-primary)]">{entry.periodEnd}</td>
                    <td className="px-4 py-2 text-[var(--cortex-text-primary)]">{entry.totalUnitsShipped.toLocaleString()}</td>
                    <td className="px-4 py-2 text-[var(--cortex-text-primary)]">{entry.activeDevices.toLocaleString()}</td>
                    <td className="px-4 py-2 text-[var(--cortex-text-secondary)]">{entry.regionBreakdown ? Object.keys(entry.regionBreakdown).join(', ') : '-'}</td>
                    <td className="px-4 py-2">
                      <button type="button" onClick={() => deleteEntry({ variables: { entryId: entry.id } })} className="text-red-500 hover:text-red-700" data-testid={`delete-base-${entry.id}`}>
                        <Trash2 size={16} />
                      </button>
                    </td>
                  </tr>
      ...
      <section data-testid="trend-section">
        ...
        {analyses.length === 0 ? (
          ...
          analyses.map((analysis) => (
            <div key={analysis.id} className="space-y-4 rounded-lg border border-[var(--cortex-border)] bg-white p-4" data-testid={`trend-analysis-${analysis.id}`}>
              <div className="flex items-center justify-between">
                <p className="text-sm text-[var(--cortex-text-secondary)]">Analysis: {new Date(analysis.analysisDate).toLocaleDateString()}</p>
              ...
              {analysis.severityDistribution && (
                <div>
                  <h3 className="mb-2 text-sm font-medium text-[var(--cortex-text-secondary)]">Severity Distribution</h3>
                  <div className="flex gap-2">
                    {Object.entries(analysis.severityDistribution).map(([key, val]) => (
                      <div key={key} className="flex-1 rounded-md bg-gray-50 p-2 text-center">
                        <p className="text-lg font-semibold text-[var(--cortex-text-primary)]">{val}</p>
              ...
              {analysis.classificationDistribution && (
                <div>
                  <h3 className="mb-2 text-sm font-medium text-[var(--cortex-text-secondary)]">Classification Distribution</h3>
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(analysis.classificationDistribution).map(([key, val]) => (
                      <span key={key} className="inline-flex rounded-full bg-blue-50 px-2.5 py-0.5 text-xs text-blue-700">{key}: {val}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {analysis.significantChanges && analysis.significantChanges.length > 0 && (
                <div>
                  <h3 className="mb-2 text-sm font-medium text-[var(--cortex-text-secondary)]">Significant Changes</h3>
              ...
              {analysis.status === 'FINALIZED' && analysis.conclusions && (
                <div className="border-t border-[var(--cortex-border)] pt-4">
                  <h3 className="mb-1 text-sm font-medium text-[var(--cortex-text-secondary)]">Conclusions</h3>
                  <p className="text-sm text-[var(--cortex-text-primary)]">{analysis.conclusions}</p>
                </div>
              )}
            </div>
          ))
