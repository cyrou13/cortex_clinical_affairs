...
export function ExtractionProgressPanel({
  ...
  const { data, loading } = useQuery(GET_EXTRACTION_PROGRESS, {
  ...
  return (
    <div className="space-y-4 rounded-lg border border-[var(--cortex-border)] p-4" data-testid="extraction-progress-panel">
      ...
      <div className="space-y-1">
        ...
        <div className="text-right text-xs font-medium text-[var(--cortex-text-muted)]" data-testid="progress-percentage">
          {percentage}%
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 text-center">
        <div className="rounded border border-[var(--cortex-border)] p-2">
          <div className="text-lg font-bold text-blue-600" data-testid="count-extracted">{extracted}</div>
          <div className="text-[10px] text-[var(--cortex-text-muted)]">Extracted</div>
        </div>
        <div className="rounded border border-[var(--cortex-border)] p-2">
          <div className="text-lg font-bold text-emerald-600" data-testid="count-reviewed">{reviewed}</div>
          <div className="text-[10px] text-[var(--cortex-text-muted)]">Reviewed</div>
        </div>
        <div className="rounded border border-[var(--cortex-border)] p-2">
          <div className="text-lg font-bold text-red-600" data-testid="count-flagged">{flagged}</div>
          <div className="text-[10px] text-[var(--cortex-text-muted)]">Flagged</div>
        </div>
        <div className="rounded border border-[var(--cortex-border)] p-2">
          <div className="text-lg font-bold text-gray-500" data-testid="count-pending">{pending}</div>
      ...
      {/* Empty state */}
      {total === 0 && (
        <div className="py-4 text-center text-sm text-[var(--cortex-text-muted)]" data-testid="empty-state">
