...
} from '../../domain/value-objects/complaint-status.js';
...
export class UpdateComplaintUseCase {
  ...
  async execute(input: UpdateComplaintInput): Promise<UpdateComplaintResult> {
    ...
    if (
      input.status &&
      !canTransitionComplaint(complaint.status as ComplaintStatus, input.status as ComplaintStatus)
    ) {
      throw new ValidationError(
        `Cannot transition complaint from ${complaint.status} to ${input.status}`,
      );
    }
    ...
    if (input.resolution !== undefined) {
      ...
      updatedFields.push('resolution');
    }
    if (input.correctiveAction !== undefined) {
      updateData.correctiveAction = input.correctiveAction;
      updatedFields.push('correctiveAction');
    }
