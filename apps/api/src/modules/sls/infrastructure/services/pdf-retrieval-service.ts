...
export class PdfRetrievalService {
  ...
  async retrieve(article: ArticleForRetrieval): Promise<PdfRetrievalResult> {
    ...
    const sources = [
      ...
      () => this.tryDoiResolution(article.doi),
    ];
  ...
  private async tryDoiResolution(doi: string | null): Promise<PdfRetrievalResult> {
    if (!doi) return { found: false, source: null, pdfBuffer: null, pdfUrl: null };

    const url = `https://doi.org/${encodeURIComponent(doi)}`;
    const response = await fetchWithTimeout(url, SOURCE_TIMEOUT, { redirect: 'follow' });
    ...
    if (contentType.includes('application/pdf')) {
      ...
      return { found: true, source: 'DOI', pdfBuffer, pdfUrl: url };
    }
